...
const orderItemSchema = z
  ...
  .refine((v) => v.menuItem || v.itemId || v.name, { message: 'menuItem or name is required' });

export const createOrderSchema = z.object({
  tableNumber: z.union([z.string().trim().min(1), z.number()]),
  status: z.enum(['ordered', 'preparing', 'served']).optional(),
  notificationPhone: z.string().trim().regex(/^\+?[0-9]{7,15}$/).optional(),
  source: z.enum(['staff', 'table', 'kiosk', 'other']).optional(),
  items: z.array(orderItemSchema).min(1)
});

export const updateOrderStatusSchema = z.object({ status: z.enum(['ordered', 'preparing', 'served']) });
